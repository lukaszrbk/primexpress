...
http://gnomad-old.broadinstitute.org/ - GnomAD - baza danych na temat mutacji i zmienności populacyjnych w sekwencji genu
...
def primer3search():
	'''Procedura wyszukująca primery w programie Primer 3.0. Jest to backup dla analogicznej procedury, wykonywanej z użyciem narzędzia Primer 3 Plus.'''
	...
	try:
		...
		hg19checkprim(rprim, lprim, keyfull, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc) #przejście do sprawdzenia obu wyszukanych primerów pod kątem polimorfizmów (funkcja działa tylko jeżeli użytkownik zażądał weryfikacji)

def primer3plussearch(prsq, keyfull, pool, plussrchdict, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc): #argumenty niezbędne do przekazania przez główny proces do pozostałych równoleglych procesów dla zgodności danych
	'''Procedura wyszukująca primery w programie Primer 3 Plus. Podstawowe narzędzie używane do tego celu. Wyszukiwanie odbywa się w osobnym procesie.'''
	pool.acquire() #inkrementacja wartości obiektu typu semafor, dla kontroli liczby równoległych procesów analizy sekwencji genu (zawsze mniejsza o 1 od liczby rdzeni procesora)
	driver = webdriver.Chrome() #utworzenie nowej instancji sterownika
	driver2=ActionChains(driver)
	...
	try:
		...
		plussrchdict[keyfull]=str(lprim)+'\\'+str(rprim)
		hg19checkprim(rprim, lprim, keyfull, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc) #przejście do sprawdzenia obu wyszukanych primerów pod kątem polimorfizmów (funkcja działa tylko jeżeli użytkownik zażądał weryfikacji)		
	driver.close() #zamnięcie okna przeglądarki dla danego procesu
	pool.release() #dekrementacja wartości semafora, umożliwiająca ewentualne zainicjowanie kolejnych, równoległych procesów

def hg19checkprim(rprim, lprim, keyfull, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc):
	"""Procedura porównująca sekwencje uzyskanych primerów z informacjami o polimorfizmach (zmiennościach populacyjnych) w bazie danych GnomAD. W przypadku wykrycia polimorfizmów o zbyt dużej (powyżej 1/7500 przypadków) częstości, przywoływana jest informacja na temat ich położenia i liczby osób z odmiennym genotypem w populacji europejskiej."""
				...
				if homnhet != 0: #uniknięcie próby dzielenia przez 0
					if (homnhet)/int(Llsnpfreq2) >= 1/8000: # informacja dla użytkownika pojawia się tylko wtedy, gdy istnieje realna szansa wpływu polimorfizmu na niepowodzenie rekacji sekwencjonowania (przyjęto, że wartością graniczną będzie częstość większa od 1 na 8000 przypadków)
						...
						if Lsnpprimindex != (len(lprim)-1): #scenariusz w przypadku gdy polimorfizm nie występuje na ostatnim miejscu sekwencji primera
							...
							if Llsnpfreq3 == '0': #informacja wyświetlana gdy nie wykryto żadnych homozygot
								print('\n'+'-'*30+'\n W sekwencji primera lewego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),lprim[:Lsnpprimindex]+'-->'+lprim[Lsnpprimindex]+'<--'+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków. Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Llsnpfreq2) +'-'*30)
							else: #informacja wyświetlana gdy wykryto genotypy homozygotyczne
								print('\n'+'-'*30+'\n W sekwencji primera lewego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),lprim[:Lsnpprimindex]+'-->'+lprim[Lsnpprimindex]+'<--'+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków (w tym %s przypadków homozygotycznych). Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Llsnpfreq2, Llsnpfreq3) +'-'*30) 
				continue
					...
					if (homnhet)/int(Rlsnpfreq2) >= 1/8000: # informacja dla użytkownika pojawia się tylko wtedy, gdy istnieje realna szansa wpływu polimorfizmu na niepowodzenie rekacji sekwencjonowania (przyjęto, że wartością graniczną będzie częstość większa od 1 na 8000 przypadków)
						...
						if Rsnpprimindex != -1: #scenariusz w przypadku gdy polimorfizm nie występuje na ostatnim miejscu sekwencji primera	
							if Rlsnpfreq3 == '0': #informacja wyświetlana gdy nie wykryto żadnych homozygot
								print('\n'+'-'*30+'\n W sekwencji primera prawego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),rprim[:Rsnpprimindex]+'-->'+rprim[Rsnpprimindex]+'<--'+rprim[Rsnpprimindex+1:]+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków. Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Rlsnpfreq2) +'-'*30)
							else: #informacja wyświetlana gdy wykryto genotypy homozygotyczne
								print('\n'+'-'*30+'\n W sekwencji primera prawego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),rprim[:Rsnpprimindex]+'-->'+rprim[Rsnpprimindex]+'<--'+rprim[Rsnpprimindex+1:]+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków (w tym %s przypadków homozygotycznych). Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Rlsnpfreq2, Rlsnpfreq3) +'-'*30)
						else: #scenariusz w przypadku gdy polimorfizm występuje na ostatnim miejscu sekwencji primera
							if Rlsnpfreq3 == '0': #informacja wyświetlana gdy nie wykryto żadnych homozygot
								print('\n'+'-'*30+'\n W sekwencji primera prawego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),rprim[:Rsnpprimindex]+'-->'+rprim[Rsnpprimindex]+'<--'+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków. Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Rlsnpfreq2) +'-'*30)
							else: #informacja wyświetlana gdy wykryto genotypy homozygotyczne
								print('\n'+'-'*30+'\n W sekwencji primera prawego (dla eksonu %s) na wskazanej poniżej pozycji:\n\n' % (keyfull),rprim[:Rsnpprimindex]+'-->'+rprim[Rsnpprimindex]+'<--'+'\n'+'\nwystępuje zmienność osobnicza w populacji europejskiej (nie wliczając populacji fińskiej) o częstości %s na %s przypadków (w tym %s przypadków homozygotycznych). Starter zaprojektowany w takim miejscu może spowodować niepowodzenie reakcji sekwencjonowania. Rozważ użycie innego zestawu primerów.\n' % (homnhet, Rlsnpfreq2, Rlsnpfreq3) +'-'*30)						
				continue
...
def hg19ucscsrch():	
	'''Procedura wyszukiwania pełnej sekwencji w wersji genomu hg19 w celu porównania z sekwencją otrzymaną w wyniku przeszukania wersji genomu hg38. Użycie wersji hg19 jest niezbędne dla interakcji z bazą danych o polimorfizmach. Fragment wykonywany w równolegle przebiegającym wątku.'''	
	global hg19exoncntctrl, seqfastahg19orig, chstart, chend, direction, ch #zmienne globalne, modyfikowane przez ten poboczny wątek, wykorzystywane do późniejszych analiz

	driver = webdriver.Chrome() #równolegle pracujący sterownik dla drugiego wątku
	...
	driver2.reset_actions()
	''' FRAGMENT TYMCZASOWO WYŁĄCZONY Z UŻYCIA ZE WZGLĘDU NA BRAK KOMPATYBILNOŚCI Z NAJNOWSZYMI WERSJAMI BAZY DANYCH UCSC
	...
	driver.close() #zamknięcie okna przeglądarki dla tego wątku



if __name__ == '__main__': #poniżej logika wykonywana tylko w przypadku głównego procesu aż do fragmentu n.join(); podczas uruchomienia pozostałych procesów inicjowane jest wyłącznie wykonanie kodu w obrębie funkcji hg19checkprim i primer3plussearch
	while True:
		...
		if not req:
			...
			break
	seqver=input('\nWprowadź oznaczenie LRG lub NM dla transkryptu tego genu [zgodnie z formatem z przykładów: LRG_292 , LRG_292t1 , NM_007294.3 , NM_007294.3t1]\n\n(zwróć szczególną uwagę na umieszczenie PRECYZYJNEJ numeracji):\n')
	...
	seqfastahg19orig=direction=chstart=chend=ch=gnomadsrc=0 #Zmienne globalne, modyfikowane (w przypadku verifyflag == 1) przez funkcję 'hg19ucscsrch'. Uruchomienie każdego z procesów wyszukiwania primerów wymaga przekazania tych zmiennych do funkcji. Ich wykorzystanie następuje jednak tylko w przypadku wartości 1 flagi verifylag.
	...
	if 'LRG' in seqver: #jeżeli użytkownik w zamiast nazwy NM_ transkryptu użył nomenklatury LRG_
		...
		if seqverappend:
			seqver=nmfromlrg+seqverappend #przypisanie go do zmiennej 'seqver' zamiast wprowadzonej pierwotnie nazwy LRG (wersja z oznaczonym wariantem transkrypcyjnym)
		else:
			seqver=nmfromlrg #przypisanie go do zmiennej 'seqver' zamiast wprowadzonej pierwotnie nazwy LRG (wersja bez oznaczonego wariantu transkrypcyjnego)

	#ewentulane wyszukiwanie pełnej sekwencji w wersji genomu hg19- od tego momentu wyszukiwania hg19 i hg38 przebiegają w dwóch wątkach
	if verifyflag == 1: #wykonywanie tylko jeżeli użytkownik zażądał weryfikacji sekwencji primerów		
		hg19exoncntctrl=0 #zmienna musi znaleźć się w zakresie globalnym ze względu na analizę wartości przez funkcję w drugim wątku
		nthpr=threading.Thread(target=hg19ucscsrch) 
		nthpr.start() #rozpoczęcie wyszukiwania w drugim wątku

	#rozpoczęcie wyszukiwania genu w genomie hg38 (wg przyjętej praktyki laboratoryjnej jest to wersja, względem której należy projektować primery)
	...
	if  verifyflag == 1:
		nthpr.join() #gdy równocześnie sekwencje analizują dwa wątki, w tym pukncie następuje ich połączenie
	...
	if verifyflag == 1: #sprawdzenie zgodności liczby eksonów poszczególnych transkryptów w przypadku, gdy użytkownik zażądał weryfikacji sekwencji
		assert hg19exoncntctrl == hg38exoncntctrl, '\n\n\nWykryto niezgodność wersji transkryptów hg19 i hg38 badanego genu. Nie jest możliwa prawidłowa kontrola polimorfizmów. Zalecane jest ponowne uruchomienie programu bez włączonej opcji weryfikacji sekwencji, lub doprecyzowanie oznaczenia transkryptu.'
	...
	for seqdict in rawexon.values():
		...
		if nwsq in seqexo or allflag == 1: #przetwarzanie fragmentu tylko w przypadku, gdy użytkownik zażądał analizy konkretnie tego eksonu, lub wszystkich eksonów kodujących w genie
			if capitalcount > 600: #zachowanie dla zbyt dużych (>600pz) eksonów, by możliwe było zaprojektowanie primerów wydajnych na całej długości łańcucha DNA		
				...
				ky1=str(nwsq)
				primdict[ky1]=primexon #dodanie pod kluczem nwsq (nwsq == nr eksonu) sekwencji z oznaczonym miejscem dla zaprojektowania primerów do słownika, używanego później w programie primer 3.0/ primer 3 plus
				nwsq+=1 #przejście do kolejnego numeru eksonu w słowniku sformatowanych sekwencji
				olsq+=1 #przejście do kolejnego numeru eksonu w słowniku niesformatowanych sekwencji	
	...
	manager = multiprocessing.Manager() # utworzenie instancji klasy Manager, która umożliwia współdzielenie obiektów pomiędzy równocześnie przebiegającymi procesami
	processes=[] #lista procesów wyszukiwania primerów
	plussrchdict=manager.dict() #słownik współdzielony pomiędzy procesami, gromadzący dane na temat wyszukanych sekwencji primerów prawych i lewych i wyświetlający uporządkowany wynik na koniec analizy
	...
	try:
		...
		for (key, prsq) in primdict.items(): #przeszukanie słownika numerów eksonów i ich sformatowanych sekwencji (ze wstawionymi nawiasami klamrowymi i wyłączonymi powtórzeniami nukleotydowymi)
			...
			if allflag == 0: #werfikacja, czy użytkownik nie zażądał zaprojektowania wszystkich CDS	
				if 'A' in key or 'B' in key or 'C' in key: #w przypadku, gdy dany ekson został podzielony na fragmenty, oznaczenie literowe każdego z nich zostaje odcięte, a numer eksonu sprawdzony pod kątem potrzeby analizy
					nkey=key[:-1]
				else:
					nkey=key #zachowanie dla niepodzielonych eksonów
				if int(nkey) in seqexo: #część wykonywana tylko jeżeli ekson jest na liście przez nas wybranych
					primer3search()
		...
		driver.close() #zamyka aktualną instancję sterownika- od tego momentu kolejno wywoływane procesy zajmą się analizą sekwencji genu
		pool=manager.Semaphore(multiprocessing.cpu_count()-1) #Utworzenie instancji klasy Semaphore- obiekt ma za zadanie nie dopuścić, aby liczba jednocześnie trwających procesów przekroczyła pomniejszoną o 1 liczbę rdzeni procesora. Przyspiesza to analizę.
		...
		for (key, prsq) in primdict.items(): #przeszukanie słownika numerów eksonów i ich sekwencji ze wstawionymi nawiasami klamrowymi
			...
			if allflag == 0: #werfikacja, czy użytkownik nie zażądał zaprojektowania wszystkich CDS		
				if 'A' in key or 'B' in key or 'C' in key: #w przypadku, gdy dany ekson został podzielony na fragmenty, oznaczenie literowe każdego z nich zostaje odcięte, a numer eksonu sprawdzony pod kątem potrzeby analizy
					...
					nkey=key #zachowanie dla niepodzielonych eksonów
				if int(nkey) in seqexo: #część wykonywana tylko jeżeli ekson jest na liście przez nas wybranych
					nth=multiprocessing.Process(target=primer3plussearch, args=(prsq, keyfull, pool, plussrchdict, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc)) #wyszukiwanie sekwencji primerów w programie Primer 3 Plus; przekazane zostają wszystkie niezbędne zmienne konieczne dla działania funkcji hg19checkprim i primer3plussearch
					processes.append(nth)
					nth.start()	#dla każdego fragmentu genu uruchamiany jest osobny proces, analizujący sekwencję
			else: #zachowanie pętli jeżeli użytkownik zażądał zaprojektowania primerów do wszystkich CDS
				nth=multiprocessing.Process(target=primer3plussearch, args=(prsq, keyfull, pool, plussrchdict, verifyflag, seqfastahg19orig, direction, chstart, chend, ch, gnomadsrc)) #wyszukiwanie sekwencji primerów w programie Primer 3 Plus; przekazane zostają wszystkie niezbędne zmienne konieczne dla działania funkcji hg19checkprim i primer3plussearch	
				processes.append(nth) 
				nth.start()	#dla każdego fragmentu genu uruchamiany jest osobny proces, analizujący sekwencję
	...
	for n in processes:
		n.join() #od tego punktu działanie programu zostaje przeniesione do jednego procesu w momencie, gdy wszystkie pozostałe wykonają swoje zadania

	for k in sorted(plussrchdict.keys(), key=lambda s: [int(t) if t.isdigit() else t.lower() for t in re.split('(\d+)', s)]):#Funkcja lambda w połączeniu z wyrażeniem regularnym sortują klucze słownika plussrchdict zgodnie z kolejnością numeracji i oznaczeniami literowymi fragmentów. Dzięki temu, mimo braku synchronizacji zakończenia działania poszczególnych procesów, użytkownik otrzymuje uporządkowany wynik.
		v=plussrchdict[k]
		sepprimer=v.index('\\') #wartości słownika to sekwencje primerów rozdzielone ukośnikami- w tym miejscu nastepuje zindeksowanie pozycji ukośnika, aby rozdzielić sekwencje lewego i prawego primera
		print('','#'*50, '\nGEN %s - EKSON %s -- SEKWENCJA LEWEGO PRIMERA: %s' %(req, k, v[:sepprimer]))
		print('GEN %s - EKSON %s -- SEKWENCJA PRAWEGO PRIMERA: %s\n' %(req, k, v[sepprimer+1:]), '#'*50)
